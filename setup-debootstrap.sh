...
debpkg_powerpcspe='systemd-sysv udev kmod dbus apt apt-utils dialog debian-ports-archive-keyring net-tools iproute2 iputils-ping ifupdown ssh nano pciutils i2c-tools dosfstools'
debpkg_s390x='kmod dbus apt apt-utils dialog net-tools iproute2 iputils-ping ifupdown openssh-client nano pciutils i2c-tools dosfstools'
...
# you can put -C to archive contents inside the dir like "tar cpfz $tar_name -C $rootfs_dir_utc/"
cd - >/dev/null
